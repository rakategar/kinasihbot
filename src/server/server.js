...
client.on('message', async (message) => {
    ...
    if (msg.includes('_')) {
        ...
        const selectedProduct = pricelists[product]?.find(item => item.code === code);
...
*Jika sudah payment ketik (konfirmasi kinasih)*`;
            ...
            const media = MessageMedia.fromFilePath(path.join(__dirname, '../../public/qris.jpg'));
...
Harga: ${order.price}`;
            ...
            try {
                ...
                try {
                    const productModule = require(`../product/${order.product.toLowerCase().slice(0, 3)}/p${order.price.toLowerCase()}.js`);
                    await productModule.handleOrder(client, message, order, adminNumber);
                } catch (err) {
                    if (err.code === 'MODULE_NOT_FOUND') {
                        console.log(`Modul untuk produk ${order.product} dengan harga ${order.price} tidak ditemukan. Melanjutkan tanpa menjalankan modul produk.`);
                    } else {
    ...
    // Proses konfirmasi pesanan selesai dari admin (!done {id})
        ...
        if (id) {
            try {
                ...
                console.error('Gagal mengambil atau mengupdate pesanan dari database:', error);
                await message.reply(`Terjadi kesalahan saat memproses pesanan dengan ID: ${id}.`);
            }
        } else {
    ...
    // Tangani pesan produk jika tidak ada perintah pembelian
        ...
        const matchedProduct = Object.keys(pricelists).find(product => msg.includes(product));

        if (matchedProduct) {
            const pricelist = pricelists[matchedProduct].map(item => `${item.code}. ${item.description} - ${item.price}`).join('\n');
...
Pesan ini dibalas oleh bot Kinasihku!`;
            ...
            console.log(`Auto-reply sent to ${message.from} for product: ${matchedProduct}`);
